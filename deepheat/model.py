...
class Model:
    ...
    def train(
       ...
       callbacks: List of ``dde.callbacks.Callback`` instances. List of callbacads
        ...
        if verbose > 0 and config.rank == 0:
            ...
            plt.figure()
            plt.plot(self.losshistory.steps, np.sum(self.losshistory.loss_train, axis=1), label="Train Loss")
            plt.plot(self.losshistory.steps, np.sum(self.losshistory.loss_test, axis=1), label="Test Loss")
            plt.xlabel("Step")
            plt.ylabel("Loss")
            plt.yscale("log")
            ...
            with open("training_loss.txt", "w") as f:
                f.write("Step\tTrain Loss\tTest Loss\n")
                for step, train_loss, test_loss in zip(
                    self.losshistory.steps,
                    np.sum(self.losshistory.loss_train, axis=1),
                    np.sum(self.losshistory.loss_test, axis=1)
                ):
                    f.write(f"{step}\t{train_loss}\t{test_loss}\n")
    ...
    def _train_sgd(self, iterations, display_every, verbose=1):
        ...
        for i in range(iterations):
            ...
            if current_time - last_save_time >= 3600:  # 每小时存一次
                self.savetime()
    ...
    def savetime(self, path=None):
        """Save model weights every hour with timestamp into a TIME/ folder."""
        # 如果没有指定 path，则使用默认 TIME 目录
        if path is None:
            save_dir = "TIME"
            os.makedirs(save_dir, exist_ok=True)  # 如果不存在则创建 TIME 文件夹
    ...
    def _test(self, verbose=1):
        ...
        if verbose > 0 and config.rank == 0:
            display.training_display(self.train_state)
            if self.check_dir:
                checkpoint_dir = self.check_dir
                os.makedirs(checkpoint_dir, exist_ok=True)
                checkpoint_path = os.path.join(checkpoint_dir, f"model_step_{self.train_state.step}.pt")
                torch.save(self.net.state_dict(), checkpoint_path)
            else:
                # 创建checkpoint文件夹并保存模型
