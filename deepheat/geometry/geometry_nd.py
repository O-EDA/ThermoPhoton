...
class Hypercube(Geometry):
    ...
    def biased_random_points(
        ...
        if biased_center is None:
            biased_center = (self.xmin[biased_dim] + self.xmax[biased_dim]) / 2.0
        if sigma is None:
            sigma = (self.xmax[biased_dim] - self.xmin[biased_dim]) / 10.0

        uniform_sample = sample(int(n*0.9), self.dim, random)
        pts_biased = (self.xmax - self.xmin) * uniform_sample + self.xmin

        def truncated_normal(n_samples, mean, sigma, lower, upper):
            samples = np.empty(n_samples)
            count = 0
            while count < n_samples:
                rem = n_samples - count
                s = np.random.normal(loc=mean, scale=sigma, size=rem)
                valid = s[(s >= lower) & (s <= upper)]
                num_valid = valid.size
                if num_valid > 0:
                    samples[count:count + num_valid] = valid[:rem]
                    count += num_valid
            return samples

        pts_biased[:, biased_dim] = truncated_normal(
            int(n*0.9),
            mean=biased_center,
            sigma=sigma,
            lower=self.xmin[biased_dim],
            upper=self.xmax[biased_dim],
        )
        n_uniform = int(0.1 * n)
        uniform_sample = sample(n_uniform, self.dim, random)
        pts_uniform = (self.xmax - self.xmin) * uniform_sample + self.xmin
        combined = np.vstack((pts_uniform, pts_biased))
