__all__ = ["ContextTimer", "LossHistory", "Model", "TrainState"]
...
def _profiling_enabled():
    return os.environ.get("DEEPHEAT_PROFILE", "0").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _wall_time():
    if backend_name == "pytorch" and torch.cuda.is_available():
        torch.cuda.synchronize()
    return time.perf_counter()


class ContextTimer:
    """Accumulate synchronized wall time when profiling is enabled."""

    def __init__(self, totals, key, enabled):
        self.totals = totals
        self.key = key
    ...
    def __enter__(self):
        if self.enabled:
            self.started = _wall_time()
        return self

    def __exit__(self, exception_type, exception_value, traceback):
        if self.enabled:
            self.totals[self.key] += _wall_time() - self.started
...
class Model:
    ...
    def __init__(self, data, net):
        ...
        self.timer_activated = _profiling_enabled()
        self.wct_dict = {
            "inputs": 0.0,
            "forward": 0.0,
            "losses": 0.0,
    ...
    def train(
        ...
        Callbacks are the supported mechanism for periodic checkpointing and
        custom logging. Training does not write implicit hourly checkpoints or
        plots into the working directory.
        """
